...
const createHeadQuarter = async (token: string, userData: CreateHeadQuarterRequest): Promise<CreateHeadquarterResponse> => {
    try {
        const response: AxiosResponse<CreateHeadquarterResponse> = await axios.post(
            'https://createheadquarter-ete7sjbqeq-uc.a.run.app', // URL da API
