...
const SearchSelect = ({ requisition, db, store, onChange }: { onChange: (value: { label?: string, value?: string, data?: any }) => void, store: TypeStoresViewList, db: Firestore, requisition: any }) => {
    const promiseOptions = (inputValue: string) =>
        new Promise<{ label: string, value: string }[]>(async (resolve) => {
            ...
            if (response.docs) {
                resolve(Object.values(response.docs).map((item: any) => ({ label: item.name, value: item._id, data: item })));
            }
        });
